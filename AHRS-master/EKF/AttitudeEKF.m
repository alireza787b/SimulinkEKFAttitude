...
if(isempty(x_apo))
    ...
    gyro_acc_init=([0;0;0]);
    acc_init=([0;0;-9.81]);
    mag_init=([0.12;-0.59;-0.72]);%[0.120000000000000 -0.590000000000000 -0.720000000000000]
    x_apo=([gyro_init;gyro_acc_init;acc_init;mag_init]);
...
if (approx_prediction==1)
    ...
    zek =(O*dt+(eye(3)))*[zex;zey;zez];
    
else
    zek =((eye(3))+O*dt+dt^2/2*O^2)*[zex;zey;zez];
...
if (approx_prediction==1)
    %e^(Odt)=I+dt*O+dt^2/2!O^2
    % so we do a first order approximation of the exponential map
    muk =(O*dt+(eye(3)))*[mux;muy;muz];
else
    muk =((eye(3))+O*dt+dt^2/2*O^2)*[mux;muy;muz];
